...
backup_agent(){
	...
	echo "[`date`] Backup file name is ${backup_file}."
	...
	if [ $? -ne 0 ]
	then
		echo "[`date`] ERROR - Couldnot complete backup of current agent."
...
rollback_agent(){
	...
	backup_fl=`ls -ltr ctm_bck_*tar | tail -1 | awk '{ print $9 }'`
        if [ ! -f ${agnt_home}/${backup_fl} ]
        then
                echo "[`date`] ERROR - Backup file not present. Rollback unsuccessfull."
		exit 1
	fi
	
	echo "[`date`] File to rollback ${backup_fl}."

	cd ${agnt_home}
	tar -xvf ${backup_fl}
        if [ $? -ne 0 ]
        then
                echo "[`date`] ERROR - Couldnot complete rollback."
                exit 1
        fi
...
upgrade_bin=DRKAI.9.0.00_HP-UX-ia64
upgrade_bin_hsh=2700519646
patch_bin=PAKAI.9.0.00.400_HP-UX-ia64_INSTALL.BIN
patch_bin_hsh=2998420800
upgrade_optn_fl=upgrade.xml
upgrade_optn_fl_hsh=1994684972
download_home="ctmagent/v9p4/hpux"

upgrade_final_verion=9
patch_final_verion=4
